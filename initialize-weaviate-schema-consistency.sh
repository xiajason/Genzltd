...
check_schema_status() {
    ...
    if [ -n "$ssh_cmd" ]; then
        if [[ "$ssh_cmd" == *"basic.pem"* ]]; then
            # 腾讯云环境
            schema_response=$(ssh -i "$ssh_cmd" "$TENCENT_SSH_USER@$weaviate_url" "curl -s ${LOCAL_WEAVIATE_URL}/v1/schema" 2>/dev/null)
        else
            # 阿里云环境
            if [[ "$ssh_cmd" == *"basic.pem"* ]]; then
            # 腾讯云环境
            schema_response=$(ssh -i "$ssh_cmd" "$TENCENT_SSH_USER@$weaviate_url"
        else
            # 阿里云环境
            schema_response=$(ssh -i "$ssh_cmd" "$ALIBABA_SSH_USER@$weaviate_url"
        fi "curl -s ${LOCAL_WEAVIATE_URL}/v1/schema" 2>/dev/null)
        fi
    else
...
cleanup_existing_schema() {
    ...
    if [ -n "$ssh_cmd" ]; then
        delete_response=$(ssh -i "$ssh_cmd" "$ALIBABA_SSH_USER@$weaviate_url" "curl -s -X DELETE ${LOCAL_WEAVIATE_URL}/v1/schema/Resume" 2>/dev/null)
    else
...
create_standard_schema() {
    ...
    if [ -n "$ssh_cmd" ]; then
        if [[ "$ssh_cmd" == *"basic.pem"* ]]; then
            ...
            # 阿里云环境
            schema_response=$(ssh -i "$ssh_cmd" "$ALIBABA_SSH_USER@$weaviate_url"
        fi "curl -s -X POST ${LOCAL_WEAVIATE_URL}/v1/schema -H 'Content-Type: application/json' -d '$RESUME_SCHEMA'" 2>/dev/null)
    else
...
get_all_schema_status() {
    ...
    local alibaba_schema=$(ssh -i "$ALIBABA_SSH_KEY" "$ALIBABA_SSH_USER@$ALIBABA_CLOUD_IP" "curl -s ${LOCAL_WEAVIATE_URL}/v1/schema" 2>/dev/null)
    local tencent_schema=$(ssh -i "$TENCENT_SSH_KEY" "$TENCENT_SSH_USER@$TENCENT_CLOUD_IP" "curl -s ${LOCAL_WEAVIATE_URL}/v1/schema" 2>/dev/null)
...
# 主执行逻辑
main() {
    echo -e "${BLUE}🔧 Weaviate Schema一致性初始化脚本 - $(date)${NC}"
    echo "======================================================"
