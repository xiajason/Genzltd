...
class AlibabaCloudDatabaseTester:
        ...
        if exit_code == 0:
            ...
            if db_config['database'] in stdout:
                ...
                if table_exit_code == 0:
                    table_count = len([line for line in table_stdout.split('\n') if line.strip() and not line.startswith('Tables_in_')])
    ...
    def test_postgresql_connection(self) -> Dict[str, Any]:
        ...
        # 简化的PostgreSQL测试
        test_query = f"docker exec {db_config['container']} psql -U {db_config['user']} -d {db_config['database']} -c '\\dt'"
        
        exit_code, stdout, stderr = self.run_ssh_command(test_query)
        
        if exit_code == 0:
            # 计算表数量
            table_count = len([line for line in stdout.split('\n') if '|' in line and 'table_name' not in line])
    ...
    def save_results(self, results: Dict[str, Any]) -> str:
        ...
        filename = f"alibaba_cloud_database_test_fixed_{timestamp}.json"
...
def main():
    """主函数"""
    print("🚀 阿里云多数据库通信连接测试和数据一致性测试 - 修复版")
