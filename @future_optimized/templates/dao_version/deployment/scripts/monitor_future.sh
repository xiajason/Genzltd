...
check_database_connections() {
    ...
    # 检查MySQL
    if docker exec dao-mysql mysqladmin ping -h localhost --silent; then
    ...
    # 检查PostgreSQL
    if docker exec dao-postgresql pg_isready -h localhost; then
    ...
    # 检查Redis
    if docker exec dao-redis redis-cli ping | grep -q PONG; then
...
check_logs() {
    ...
    # 检查各服务的最近日志
    services=("dao-mysql" "dao-postgresql" "dao-redis" "dao-neo4j" "dao-elasticsearch" "dao-weaviate")
...
run_health_check() {
    ...
    # 执行数据库验证脚本
    if [ -f "scripts/dao_database_verification_script.py" ]; then
