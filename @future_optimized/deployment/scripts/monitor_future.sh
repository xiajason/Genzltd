...
check_database_connections() {
    ...
    # 检查MySQL
    if docker exec future-mysql mysqladmin ping -h localhost --silent; then
    ...
    # 检查PostgreSQL
    if docker exec future-postgresql pg_isready -h localhost; then
    ...
    # 检查Redis
    if docker exec future-redis redis-cli ping | grep -q PONG; then
...
check_logs() {
    ...
    # 检查各服务的最近日志
    services=("future-mysql" "future-postgresql" "future-redis" "future-neo4j" "future-elasticsearch" "future-weaviate")
...
run_health_check() {
    ...
    # 执行数据库验证脚本
    if [ -f "scripts/future_database_verification_script.py" ]; then
