...
test_configuration_consistency() {
    ...
    # 检查腾讯云Docker服务
    if ssh -i ~/.ssh/basic.pem ubuntu@101.33.251.158 "docker ps | grep -E '(dao-web|blockchain-web|dao-postgres|dao-redis)'" > /dev/null 2>&1; then
...
generate_final_report() {
    local report_file="final-data-consistency-test-report-$(date +%Y%m%d_%H%M%S).md"
...
EOF
    ...
    echo -e "${GREEN}📄 最终测试报告已生成: $report_file${NC}"
}

# 主执行函数
main() {
    echo -e "${BLUE}🚀 开始最终三环境数据一致性测试...${NC}"
