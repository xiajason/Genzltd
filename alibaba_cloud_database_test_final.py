...
class AlibabaCloudDatabaseTester:
    ...
    def test_mysql_connection(self) -> Dict[str, Any]:
        ...
        # 测试数据库连接
        test_query = f"docker exec {db_config['container']} mysql -u root -p{db_config['password']} -e 'SHOW DATABASES;'"
        
        exit_code, stdout, stderr = self.run_ssh_command(test_query)
        
        if exit_code == 0:
            # 检查是否包含目标数据库
            if db_config['database'] in stdout:
                # 测试表数量
                table_query = f"docker exec {db_config['container']} mysql -u root -p{db_config['password']} -e 'USE {db_config['database']}; SHOW TABLES;'"
                table_exit_code, table_stdout, table_stderr = self.run_ssh_command(table_query)
                
                table_count = 0
                if table_exit_code == 0:
                    # 计算表数量，排除表头
                    lines = table_stdout.split('\n')
                    table_count = len([line for line in lines if line.strip() and not line.startswith('Tables_in_') and 'mysql:' not in line])
                ...
                return {
                    "status": "failed",
                    "error": f"Database {db_config['database']} not found",
                    "connection": "Docker exec"
                }
        else:
            return {
    ...
    def test_postgresql_connection(self) -> Dict[str, Any]:
        ...
        # 测试PostgreSQL连接
        test_query = f"docker exec {db_config['container']} psql -U {db_config['user']} -d {db_config['database']} -c \"SELECT table_name FROM information_schema.tables WHERE table_schema = 'public';\""
        ...
        if exit_code == 0:
            # 计算表数量
            lines = stdout.split('\n')
            table_count = len([line for line in lines if '|' in line and 'table_name' not in line and line.strip()])
    ...
    def test_neo4j_connection(self) -> Dict[str, Any]:
        ...
        # 测试Neo4j连接
        test_query = f"docker exec {db_config['container']} cypher-shell -u {db_config['user']} -p {db_config['password']} 'RETURN 1 as test;'"
        
        exit_code, stdout, stderr = self.run_ssh_command(test_query)
        
        if exit_code == 0:
            # 获取节点数量
            node_query = f"docker exec {db_config['container']} cypher-shell -u {db_config['user']} -p {db_config['password']} 'MATCH (n) RETURN COUNT(n) as node_count;'"
            node_exit_code, node_stdout, node_stderr = self.run_ssh_command(node_query)
            
            node_count = 0
            if node_exit_code == 0:
                try:
                    lines = node_stdout.strip().split('\n')
                    for line in lines:
    ...
    def test_elasticsearch_connection(self) -> Dict[str, Any]:
        ...
        if exit_code == 0 and stdout.strip():
            ...
            # 如果健康检查失败，尝试基本连接
            basic_query = f"curl -s http://localhost:{db_config['port']}/"
    ...
    def save_results(self, results: Dict[str, Any]) -> str:
        ...
        filename = f"alibaba_cloud_database_test_final_{timestamp}.json"
...
def main():
    """主函数"""
    print("🚀 阿里云多数据库通信连接测试和数据一致性测试 - 最终修复版")
