...
# 临时文件存储测试数据
TEMP_DIR="/tmp/advanced_load_balance_test_$$"
mkdir -p "$TEMP_DIR"
...
send_request() {
    ...
    if [ "$response" = "200" ]; then
        ...
        echo "✅ 请求 $request_id -> 端口 $port: 成功 (${response_time}s)"
    else
...
calculate_load_balance() {
    ...
    if [ $max_deviation -lt 15 ]; then
        log_success "负载均衡效果: 优秀 (偏差 < 15%)"
    elif [ $max_deviation -lt 25 ]; then
        log_warning "负载均衡效果: 良好 (偏差 < 25%)"
    else
        log_error "负载均衡效果: 需要优化 (偏差 >= 25%)"
    fi
}
...
# 主函数
main() {
    log_info "Basic Server集群高级负载均衡测试开始"
