...
show_log_status() {
    ...
    LOGS_DIR="/Users/szjason72/zervi-basic/basic/logs"
    
    for node in 1 2 3; do
        log_file="$LOGS_DIR/basic-server-node$node.log"
        if [ -f "$log_file" ]; then
            size=$(ls -lh "$log_file" | awk '{print $5}')
            lines=$(wc -l < "$log_file")
            log_success "Node $node 日志: $log_file (大小: $size, 行数: $lines)"
        else
            log_warning "Node $node 日志文件不存在"
        fi
    done
...
main() {
    ...
    check_node_status "Basic Server Node 2" "8180"
    check_node_status "Basic Server Node 3" "8280"
    show_log_status
