...
cleanup_logs() {
    ...
    if [ -d "$LOGS_DIR" ]; then
        rm -f "$LOGS_DIR/basic-server-node*.log"
        rm -f "$LOGS_DIR/basic-server-node*.pid"
        log_success "日志文件已清理"
    else
        log_warning "日志目录不存在"
    fi
...
show_stop_status() {
    ...
    if lsof -i :8180 > /dev/null 2>&1; then
        log_warning "端口8180仍被占用"
    else
        log_success "端口8180已释放"
    fi
    
    if lsof -i :8280 > /dev/null 2>&1; then
        log_warning "端口8280仍被占用"
    else
        log_success "端口8280已释放"
    fi
