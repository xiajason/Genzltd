...
start_basic_server_instances() {
    ...
    CONFIG_FILE="$BACKEND_DIR/configs/cluster-config-node1.yaml" \
    ...
    # 启动Node 2 (端口8180)
    log_info "启动Basic Server Node 2 (端口8180)..."
    NODE_ID="basic-server-node-2" \
    NODE_PORT="8180" \
    CLUSTER_MANAGER_URL="http://localhost:9091" \
    CONFIG_FILE="$BACKEND_DIR/configs/cluster-config-node2.yaml" \
    ...
    # 启动Node 3 (端口8280)
    log_info "启动Basic Server Node 3 (端口8280)..."
    NODE_ID="basic-server-node-3" \
    NODE_PORT="8280" \
    CLUSTER_MANAGER_URL="http://localhost:9091" \
    CONFIG_FILE="$BACKEND_DIR/configs/cluster-config-node3.yaml" \
...
show_cluster_status() {
    ...
    # Node 2状态
    echo "Basic Server Node 2 (端口8180):"
    curl -s http://localhost:8180/api/v1/cluster/status | jq '.status.cluster_id, .status.total_nodes, .status.active_nodes' 2>/dev/null || echo "  状态获取失败"
    echo ""
    
    # Node 3状态
    echo "Basic Server Node 3 (端口8280):"
    curl -s http://localhost:8280/api/v1/cluster/status | jq '.status.cluster_id, .status.total_nodes, .status.active_nodes' 2>/dev/null || echo "  状态获取失败"
    echo ""
    ...
    echo "  Node 2 集群状态: http://localhost:8180/api/v1/cluster/status"
    echo "  Node 3 集群状态: http://localhost:8280/api/v1/cluster/status"
    echo ""
    echo "日志文件:"
