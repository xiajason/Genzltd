...
check_ssh_connection() {
    ...
    if ssh -i "$SSH_KEY" -o ConnectTimeout=10 -o BatchMode=yes "$TENCENT_USER@$TENCENT_SERVER" "echo 'SSH连接成功'" > /dev/null 2>&1; then
        ...
        log_error "无法连接到腾讯云服务器，请检查SSH配置"
        exit 1
    fi
}
...
# 检查MySQL服务状态
check_mysql_status() {
    log_info "检查腾讯云服务器MySQL服务状态..."
    if ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo mysqladmin ping -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' --silent" > /dev/null 2>&1; then
...
# 创建E2E测试数据库
create_e2e_database() {
    log_info "在腾讯云服务器创建E2E测试数据库: $DB_NAME"
    
    # 删除现有数据库（如果存在）
    ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' -e 'DROP DATABASE IF EXISTS $DB_NAME;'" > /dev/null 2>&1
    
    # 创建新数据库
    ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' -e 'CREATE DATABASE $DB_NAME CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;'" > /dev/null 2>&1
...
create_tables() {
    ...
    # 用户表
    ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' '$DB_NAME'" <<EOF
CREATE TABLE users (
...
EOF
    ...
    # 企业表
    ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' '$DB_NAME'" <<EOF
...
# 插入测试数据
insert_test_data() {
    log_info "插入腾讯云E2E测试数据..."
    
    # 插入测试用户
    ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' '$DB_NAME'" <<EOF
INSERT INTO users (username, email, password_hash, full_name, role, points) VALUES
('tencent_test_user1', 'tencent_test_user1@example.com', '\$2a\$12\$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj4J/8KzKz2a', '腾讯测试用户1', 'user', 1000),
('tencent_test_user2', 'tencent_test_user2@example.com', '\$2a\$12\$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj4J/8KzKz2a', '腾讯测试用户2', 'user', 1000),
('tencent_test_admin', 'tencent_admin@example.com', '\$2a\$12\$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj4J/8KzKz2a', '腾讯测试管理员', 'admin', 1000);
EOF
...
# 验证数据
verify_data() {
    log_info "验证腾讯云E2E测试数据..."
    
    USER_COUNT=$(ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' '$DB_NAME' -s -e 'SELECT COUNT(*) FROM users;'")
    COMPANY_COUNT=$(ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' '$DB_NAME' -s -e 'SELECT COUNT(*) FROM companies;'")
    JOB_COUNT=$(ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' '$DB_NAME' -s -e 'SELECT COUNT(*) FROM jobs;'")
    CATEGORY_COUNT=$(ssh -i "$SSH_KEY" "$TENCENT_USER@$TENCENT_SERVER" "sudo mysql -h'$DB_HOST' -P'$DB_PORT' -u'$DB_USER' '$DB_NAME' -s -e 'SELECT COUNT(*) FROM job_categories;'")

    log_success "腾讯云E2E测试数据库设置完成！"
    log_info "数据统计:"
    ...
    log_info "  - 数据库主机: $TENCENT_SERVER:$DB_PORT"
}
...
main() {
    ...
    echo ""
    log_success "腾讯云E2E测试数据库初始化完成！"
    log_info "现在可以启动腾讯云E2E测试服务了"
    log_info "服务器地址: $TENCENT_SERVER"
    log_info "数据库名称: $DB_NAME"
}
