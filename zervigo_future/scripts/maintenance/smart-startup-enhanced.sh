...
cleanup_docker_images() {
    ...
    log_info "开始Docker清理流程（保留AI服务容器）..."
    
    # 1. 清理停止的容器（排除AI服务容器）
    log_info "清理停止的容器（保留AI服务容器）..."
    ...
    # 4. 清理未使用的网络（保留AI服务网络）
    log_info "清理未使用的网络（保留AI服务网络）..."
    docker network prune -f > /dev/null 2>&1 || true
    log_success "已清理未使用的网络"
    
    # 5. 清理未使用的卷（保留AI服务卷）
    log_info "清理未使用的卷（保留AI服务卷）..."
    ...
    local ai_containers=$(docker ps --format "{{.Names}}" | grep -E "(jobfirst-ai|jobfirst-mineru|jobfirst-models|jobfirst-monitor)" | wc -l)
    log_info "  镜像数量: $total_images"
    log_info "  容器数量: $total_containers"
    log_info "  AI服务容器: $ai_containers"
    
    log_success "Docker清理完成（AI服务容器已保留）"
}
...
3. Docker资源清理 - 自动清理悬空镜像、停止容器、未使用资源，保留AI服务容器
4. AI服务容器管理 - 智能检查并启动AI服务容器，不停止现有容器
5. 智能日志管理 - 日志归档、清理、压缩
...
启动流程:
  ...
  8. 启动本地AI服务
  9. 启动容器化AI服务 (检查并启动AI服务容器，不停止现有容器)
  10. 智能日志管理
  11. 验证服务状态
  12. 生成启动报告
