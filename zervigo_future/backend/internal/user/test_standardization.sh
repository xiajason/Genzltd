...
log_info "开始测试 User Service 标准化版本..."
...
# 检查认证管理
if grep -q "auth/register" main_standardized.go; then
    log_success "保持认证管理功能"
else
    log_error "缺少认证管理功能"
fi

# 检查用户管理
if grep -q "users.GET" main_standardized.go; then
    log_success "保持用户管理功能"
else
    log_error "缺少用户管理功能"
fi

# 检查角色管理
if grep -q "roles.GET" main_standardized.go; then
...
# 检查权限管理
if grep -q "permissions.GET" main_standardized.go; then
...
# 检查简历权限管理
if grep -q "resume-permissions" main_standardized.go; then
    log_success "保持简历权限管理功能"
else
    log_error "缺少简历权限管理功能"
fi

# 检查利益相关方管理
if grep -q "stakeholders" main_standardized.go; then
    log_success "保持利益相关方管理功能"
else
    log_error "缺少利益相关方管理功能"
fi

# 检查评论管理
if grep -q "comments" main_standardized.go; then
    log_success "保持评论管理功能"
else
    log_error "缺少评论管理功能"
fi

# 检查分享管理
if grep -q "shares" main_standardized.go; then
    log_success "保持分享管理功能"
else
    log_error "缺少分享管理功能"
fi

# 检查积分管理
if grep -q "points" main_standardized.go; then
    log_success "保持积分管理功能"
else
    log_error "缺少积分管理功能"
fi
...
# 7. 总结
log_info "7. 测试总结..."
log_success "User Service 标准化测试完成"
